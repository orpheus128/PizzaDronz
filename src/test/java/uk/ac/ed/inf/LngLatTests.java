...
//    }
    ...
    //if the angle is 999 the drone should be in the same position
    @RepeatedTest(100)
    void hovering() {
        LngLatHandler handler = new LngLatHandler();
    ...
    //tests if the nextPosition function returns the correct position
    @RepeatedTest(100)
    void flying() {
        LngLatHandler handler = new LngLatHandler();
        ...
        double randomLng = -180 + new Random().nextDouble() * 360;
        double randomLat = -90 + new Random().nextDouble() * 180;
        double randomAngle = 45 * new Random().nextInt(8);
